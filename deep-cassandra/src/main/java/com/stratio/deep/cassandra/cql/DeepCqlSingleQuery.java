...
public class DeepCqlSingleQuery implements AutoCloseable {
     ...
     *            the Cells object containing all the other row columns.
     */
    public void writeMaxStatement(Cells keys, Cells values, String columnName) {
    	
        /* generate SQL */
        String localCql;
        
        StringBuilder sb = new StringBuilder("UPDATE ").append(writeConfig.getKeyspace()).append(".").append(quote(writeConfig.getColumnFamily()))
                .append(" SET ");
        ...
        sb.append(quote(columnName)).append(" = ").append(quote(columnName)).append("+").append("?");
        //sb.append(quote(columnName)).append(" = ").append("?");
