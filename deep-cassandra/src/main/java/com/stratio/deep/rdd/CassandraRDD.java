...
import com.stratio.deep.thrift.ThriftRangeUtils;
...
/**
 * Base class that abstracts the complexity of interacting with the Cassandra
 * Datastore.<br/>
...
public abstract class CassandraRDD<T> extends RDD<T> {
     ...
     * successfully. This class simply closes the
     * {@link org.apache.cassandra.hadoop.cql3.CqlPagingRecordReader} passed as
     * an argument.
     * 
     * @param <R>
    ...
    class OnComputedRDDCallback<R> extends AbstractFunction0<R> {
        ...
        public OnComputedRDDCallback(DeepRecordReader recordReader,
                DeepPartition dp) {
            super();
    ...
    /**
     * Persists the given RDD to the underlying Cassandra datastore using the
     * java cql3 driver.<br/>
     * Beware: this method does not perform a distributed write as
     * {@link com.stratio.deep.rdd.CassandraRDD#saveRDDToCassandra} does, uses
     * the Datastax Java Driver to perform a batch write to the Cassandra
     * server.<br/>
     * This currently scans the partitions one by one, so it will be slow if a
     * lot of partitions are required.
     * 
     * @param rdd
     *            the RDD to persist.
     * @param writeConfig
     *            the write configuration object.
     */
    @SuppressWarnings("unchecked")
    public static <W, T extends IDeepType> void cql3SaveRDDToCassandra(
            RDD<W> rdd, ICassandraDeepJobConfig<W> writeConfig) {
        if (IDeepType.class.isAssignableFrom(writeConfig.getEntityClass())) {
            ...
            CassandraRDDUtils.doCql3SaveToCassandra(r, c,
                    new DeepType2TupleFunction<T>());
    ...
    /**
     * Persists the given RDD of Cells to the underlying Cassandra datastore,
     * using configuration options provided by <i>writeConfig</i>.
     * 
     * @param rdd
     *            the RDD to persist.
     * @param writeConfig
    ...
    public static <W, T extends IDeepType> void saveRDDToCassandra(RDD<W> rdd,
        ...
        if (IDeepType.class.isAssignableFrom(writeConfig.getEntityClass())) {
            ...
            CassandraRDDUtils.doSaveToCassandra(r, c,
                    new DeepType2TupleFunction<T>());
        } else if (Cells.class.isAssignableFrom(writeConfig.getEntityClass())) {
            ICassandraDeepJobConfig<Cells> c = (ICassandraDeepJobConfig<Cells>) writeConfig;
            RDD<Cells> r = (RDD<Cells>) rdd;
     ...
     *            the write configuration object.
     * @param <W>
     *            the generic type associated to the provided configuration
     *            object.
     */
    public static <W> void saveRDDToCassandra(JavaRDD<W> rdd,
            ICassandraDeepJobConfig<W> writeConfig) {
        saveRDDToCassandra(rdd.rdd(), writeConfig);
    }

    /**
     * Public constructor that builds a new Cassandra RDD given the context and
     * the configuration file.
     * 
     * @param sc
     *            the spark context to which the RDD will be bound to.
     * @param config
     *            the deep configuration object.
     */
    @SuppressWarnings("unchecked")
    public CassandraRDD(SparkContext sc, ICassandraDeepJobConfig<T> config) {
        super(sc, scala.collection.Seq$.MODULE$.empty(), ClassTag$.MODULE$
                .<T> apply(config.getEntityClass()));
        this.config = sc.broadcast(config, ClassTag$.MODULE$
                .<ICassandraDeepJobConfig<T>> apply(config.getClass()));
    }
    ...
    public Iterator<T> compute(Partition split, TaskContext ctx) {
        ...
        this.log().debug("Executing compute for split: " + deepPartition);

        final DeepRecordReader recordReader = this.initRecordReader(ctx,
                deepPartition);
        ...
        java.util.Iterator<T> recordReaderIterator = new java.util.Iterator<T>() {
            ...
            @Override
            public T next() {
                return CassandraRDD.this.transformElement(recordReader.next());
            }
        ...
        return new InterruptibleIterator<T>(ctx,
                asScalaIterator(recordReaderIterator));
    }

    /**
     * Gets an instance of the callback that will be used on the completion of
     * the computation of this RDD.
     * 
     * @param recordReader
     *            the deep record reader.
     * @param dp
     *            the spark deep partition.
     * @return an instance of the callback that will be used on the completion
     *         of the computation of this RDD.
     */
    protected AbstractFunction0<BoxedUnit> getComputeCallback(
            DeepRecordReader recordReader, DeepPartition dp) {
    ...
    public Partition[] getPartitions() {
        ...
        for (DeepTokenRange split : underlyingInputSplits) {
            partitions[i] = new DeepPartition(this.id(), i, split);

            this.log().debug("Detected partition: " + partitions[i]);
            ++i;
        }
    ...
    public Seq<String> getPreferredLocations(Partition split) {
        ...
        this.log().debug("getPreferredLocations: " + p);

        return asScalaBuffer(locations);
    }

    /**
     * Instantiates a new deep record reader object associated to the provided
     * partition.
     * 
     * @param ctx
     *            the spark task context.
     * @param dp
     *            a spark deep partition
     * @return the deep record reader associated to the provided partition.
     */
    private DeepRecordReader initRecordReader(TaskContext ctx,
            final DeepPartition dp) {
        DeepRecordReader recordReader = new DeepRecordReader(
                this.config.value(), dp.splitWrapper());
        ctx.addOnCompleteCallback(this.getComputeCallback(recordReader, dp));
        return recordReader;
