...
/**
 * Defines the public methods that each Stratio Deep Cassandra configuration
 * object should implement.
 */
public interface ICassandraDeepJobConfig<T> extends
        IDeepJobConfig<T, ICassandraDeepJobConfig<T>> {

    /**
     * Adds a new filter for the Cassandra underlying datastore.<br/>
     * Once a new filter has been added, all subsequent queries generated to the
     * underlying datastore will include the filter on the specified column
     * called <i>filterColumnName</i>. Before propagating the filter we check if
     * an index exists in Cassandra.
     * 
     * @param filterColumnName
     *            the name of the columns (as known by the datastore) to filter
     *            on.
     * @param filterValue
     *            the value of the filter to use. May be any expression, depends
     *            on the actual index implementation.
     * @return this configuration object.
     * @throws com.stratio.deep.exception.DeepIndexNotFoundException
     *             if the specified field has not been indexed in Cassandra.
     * @throws com.stratio.deep.exception.DeepNoSuchFieldException
     *             if the specified field is not a valid column in Cassandra.
     */
    ...
    /**
     * Let's the user specify an alternative partitioner class. The default
     ...
     * @return this object.
     */
    public abstract ICassandraDeepJobConfig<T> partitioner(
            String partitionerClassName);

    /**
     * Returns the session opened to the cassandra server.
     * 
     * @return the Session opened by this configuration object to the cassandra
     *         server.
     */
    ...
    /**
     * RPC port where the remote Cassandra cluster is listening to. Defaults to
     * 9160.
     * 
    ...
    /**
     * CQL port where the remote Cassandra cluster is listening to. Defaults to
     * 9042.
     * 
     * @return the cql port.
     */
     ...
     * Can be one of those consistency levels defined in
     * {@link org.apache.cassandra.db.ConsistencyLevel}.<br/>
     * Defaults to {@link org.apache.cassandra.db.ConsistencyLevel#LOCAL_ONE}.
     * 
     * @param level
     *            the read consistency level to use.
     * @return this configuration object.
     */
     ...
     * 
     * @param level
     *            the write consistency level to use.
     * @return this configuration object.
     */
    public abstract ICassandraDeepJobConfig<T> writeConsistencyLevel(
            String level);
    ...
    /**
     * Sets the token range bisect factor. The provided number must be a power
     * of two. Defaults to 1.
     * 
     * @param bisectFactor
     *            the bisect factor to use.
     * @return this configuration object.
     */
    ...
    /**
     * Sets the underlying datastore table or collection from which data will be
     * read from.
     * 
     * @param table
     *            the table name.
    ...
    /**
     * Returns the name of the configured column family. Column family name is
     * case sensitive.
     * 
    ...
    /**
     * Sets the cassandra CF from which data will be read from. Column family
    ...
    /**
     * Returns whether this configuration config is suitable for writing out
     * data to the datastore.
     * 
     * @return true if this configuratuon object is suitable for writing to
     *         cassandra, false otherwise.
     */
    ...
    /**
     * Returns the batch size used for writing objects to the underying
     * Cassandra datastore.
     * 
    ...
    /**
     * Returns whether or not in this configuration object we specify to
     * automatically create the output column family.
     * 
     * @return true if this configuration object has been configured to create
     *         missing tables on writes.
     */
     ...
     *            a boolean that tells this configuration obj to create missing
     *            tables on write.
     * @return this configuration object.
     */
    public abstract ICassandraDeepJobConfig<T> createTableOnWrite(
    ...
    /**
     * Sets the session to use. If a session is not provided, this object will
     * open a new one.
     * 
     * @param session
     *            the session to use.
