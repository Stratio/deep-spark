...
final object GroupingEntityWithMongoDB {
  ...
  def doMain(args: Array[String]) {
    ...
    val inputConfigEntity: MongoDeepJobConfig[BookEntity] = MongoConfigFactory.createMongoDB(classOf[BookEntity]).host(host).database(database).collection(inputCollection).initialize
