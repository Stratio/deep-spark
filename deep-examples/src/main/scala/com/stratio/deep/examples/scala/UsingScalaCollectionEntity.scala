...
object UsingScalaCollectionEntity {
  ...
  private def doMain(args: Array[String], deepContext: DeepSparkContext, p: ContextProperties) = {
    ...
    val config: CassandraDeepJobConfig[ScalaCollectionEntity] = CassandraConfigFactory.create(classOf[ScalaCollectionEntity])
    ...
    val writeConfig: CassandraDeepJobConfig[ScalaOutputEntity] = CassandraConfigFactory.create(classOf[ScalaOutputEntity])
    ...
    DeepSparkContext.saveRDD(outputRDD, writeConfig)
  }
