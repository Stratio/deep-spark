...
import com.stratio.deep.util.CellList2TupleFunction;
 ...
 */
public abstract class CassandraGenericRDD<T> extends RDD<T> {
     ...
     * @param writeConfig
     */
    public static void saveCellRDDToCassandra(RDD<Cells> rdd, IDeepJobConfig<Cells> writeConfig) {
	doSaveToCassandra(rdd, writeConfig, new CellList2TupleFunction());
    }
     ...
     * @param <W>
     */
    public static <W extends IDeepType> void saveEntityRDDToCassandra(RDD<W> rdd, IDeepJobConfig<W> writeConfig) {
	doSaveToCassandra(rdd, writeConfig, new DeepType2TupleFunction<W>());
    }

    @SuppressWarnings("unchecked")
    public CassandraGenericRDD(SparkContext sc, IDeepJobConfig<T> config) {
